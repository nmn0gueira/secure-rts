...
public class PqShpClientProtocol {

    private static final Logger LOGGER = Logger.getLogger(PqShpClientProtocol.class.getName());
    ...
    public PqShpClientProtocol(PqShpCryptoSpec cryptoSpec, KeyStore trustStore,
    ...
    public ShpMessage buildClientHello() throws GeneralSecurityException {
        clientNonce = PqShpCryptoSpec.generateNonce();
        byte[] clientCertificate = cryptoSpec.getCertificateBytes();
        // reuse clientEcdhPublicKey field to carry the ML-KEM public key
        byte[] clientKemPublicKey = cryptoSpec.getKemPublicKeyBytes();

        ShpClientHello hello = new ShpClientHello(
                request, clientCertificate, clientKemPublicKey,
                new ArrayList<>(supportedSuites.keySet()), clientNonce);
        hello.setSignature(cryptoSpec.sign(hello.bytesToSign()));

        LOGGER.info("Sent PQ CLIENT_HELLO with suites: " + new ArrayList<>(supportedSuites.keySet()));
    ...
    private ShpProtocolResult handleServerHello(ShpServerHello msg) {
        LOGGER.info("Received PQ SERVER_HELLO.");
        try {
            ...
            // serverEcdhPublicKey field carries the ML-KEM ciphertext
            byte[] sharedSecret = cryptoSpec.decapsulate(msg.serverEcdhPublicKey());
            ...
            ShpCssp cssp = new ShpCssp(encryptedPayload, integrityProof);
            LOGGER.info("Sent CSSP.");
            return ShpProtocolResult.finished(cssp.toShpMessage(makeHeader(MsgType.CSSP)));

        } catch (GeneralSecurityException e) {
            LOGGER.log(Level.SEVERE, "Error handling PQ SERVER_HELLO.", e);
            return ShpProtocolResult.error();
        }
    }
