...
public class ShpServerProtocol {
    ...
    public ShpServerProtocol(ShpCryptoSpec cryptoSpec, Map<String, User> userDatabase,
    ...
    /*private ShpProtocolResult handleType1(Type1Message msg) {
        ...
        if (currentUser == null) { LOGGER.warning("User not found: " + userId); return ShpProtocolResult.error(); }
        ...
        int iterations = ((iterationBytes[0] & 0xFF) << 8) | (iterationBytes[1] & 0xFF);
        try {
        ...
        ShpMessage response = new ShpMessage(makeHeader(MsgType.TYPE_2), salt, iterationBytes, nonce);
    ...
    /*private ShpProtocolResult handleType3(Type3Message msg) {
        ...
        try {
            byte[] dataToVerify = Utils.concat(msg.passwordEncryptedData(), msg.ydhClient(), msg.clientSignature());
            ...
            Type3Message.DecryptedContent content = Type3Message.DecryptedContent.parse(decryptedData, userIdLength);

            byte[] signatureData = Utils.concat(decryptedData, msg.ydhClient());
            if (!cryptoSpec.verifySignature(clientPublicKey, signatureData, msg.clientSignature())) {
            ...
            if (validRequests != null && !validRequests.contains(new String(content.request()))) {
            ...
            this.sharedKeyCipher = CipherSuiteFactory.sharedKeyCipher(HashUtils.SHA3_256.digest(dhSecret));

            byte[] confirmation = ShpCryptoSpec.REQUEST_CONFIRMATION.getBytes();
            byte[] incrementedClientNonce = Utils.getIncrementedBytes(content.clientNonce());
    ...
    /*private ShpProtocolResult handleType5(Type5Message msg) {
        ...
        try {
            if (!hmac.verifyIntegrity(msg.sharedKeyEncryptedData(), null, msg.integrityProof())) {
